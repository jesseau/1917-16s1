...
// test whether point is above, on or below a given line
int lineTest(double px, double py, double m, double b) {
    int retVal;

    if( py < m * px + b ) {
        retVal = BELOW_LINE; // point is under line
    } else if ( py == m * px + b ) {
        retVal = ON_LINE;
    } else {
        retVal = ABOVE_LINE; // over
    }
...
int pointTest( double px, double py, double m,double b, double lx,double ly) {
    //return (test2( px,py, m,b )==test2(lx,ly,m,b));
    int retVal = FALSE;
    
    int lineTest1 = lineTest(px, py, m, b);
    int lineTest2 = lineTest(lx, ly, m, b);

    if (lineTest1 == lineTest2) {
        retVal = TRUE;
    }
...
int isInTriangle(double x0,double y0,double x1,double y1,double x2,double y2,double px, double py) {
...
// get input
    ...
    scanned += scanf("%lf,%lf", &x0,&y0);
    printf("Triangle Vertex  B (x,y): ");
    scanned += scanf("%lf,%lf", &x1,&y1);
    printf("Triangle Vertex  C (x,y): ");
    scanned += scanf("%lf,%lf", &x2,&y2);
    printf("Test Point (x,y): ");
    scanned += scanf("%lf,%lf", &px,&py);
 
    assert(scanned == NUM_INPUT);
